...
import { DEFAULT_CLOTHING_SLOTS, DEFAULT_ACCESSORY_SLOTS } from './Constants.js';
export class AutoOutfitSystem {
    ...
    getDefaultPrompt() {
        return `${this.getDefaultOutfitStatePrompt()} ${this.getDefaultAutoOutfitPrompt()}`;
    }
    getDefaultOutfitStatePrompt() {
        return `
Here is what {{char}} is currently wearing:

**{{char}}'s Current Outfit**
${makeVarStrings('<BOT>', DEFAULT_CLOTHING_SLOTS).join('\n')}

**{{char}}'s Accessories**
${makeVarStrings('<BOT>', DEFAULT_ACCESSORY_SLOTS).join('\n')}

Here is what {{user}} is currently wearing:

**{{user}}'s Current Outfit**
${makeVarStrings('User', DEFAULT_CLOTHING_SLOTS).join('\n')}

**{{user}}'s Accessories**
${makeVarStrings('User', DEFAULT_ACCESSORY_SLOTS).join('\n')}

Outfit and accessory slots are structural fields only.
Treat "None" as meaning no item is present, not that the character possesses or lacks any physical features.
Rely on the character card and narrative context for information about anatomy, identity, and appearance.`;
    }
    getDefaultAutoOutfitPrompt() {
        return `
Analyze the character's actions in the recent messages. If the character puts on, wears, removes, or changes any clothing items, output the appropriate outfit commands.
...
Available clothing slots: ${DEFAULT_CLOTHING_SLOTS.join(', ')}
Available accessory slots: ${DEFAULT_ACCESSORY_SLOTS.join(', ')}
...
You can use "change" to change an already worn item with another item.
You can also use "change to change state of the same item you are wearing.
For example if you are wearing "White Button Blouse" and in story you unbuttoned the front, you can change it into "White Button Blouse (unbuttoned front)"
...
Important: Always use the exact slot names listed above. Never invent new slot names.`;
    ...
    getLastMessages(count = 3) {
        try {
            ...
            const validMessages = chat.filter(msg => msg && typeof msg === 'object' &&
            ...
            return recentMessages.map(msg => `${msg.is_user ? 'User' : (msg.name || 'AI')}: ${msg.mes}`).join('\n');
        }
        catch (error) {
