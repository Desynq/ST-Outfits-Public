...
export async function showImagePicker(options) {
    ...
    for (const state of imageStates) {
        ...
        const onLongPress = async () => {
            const confirm = await multiConfirm('Delete this image?', 'This action cannot be undone. Confirm?');
            if (!confirm)
                return;
            await onDelete(tag);
            selected = null;
        ...
        addLongPressAction(wrapper, 300, onLongPress, { onReleaseAfterNormalPress });
