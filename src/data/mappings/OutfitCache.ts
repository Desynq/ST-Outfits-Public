...
import { OutfitCachedDiff, OutfitCachedSnapshot } from "../model/Outfit.js";



export function normalizeOutfitSnapshots(snapshots: Record<string, any>): void {
...
function coerceOutfitSnapshot(x: unknown): x is OutfitCachedSnapshot {
	if (!isRecord(x)) return false;

	if (typeof x.namespace !== 'string') return false;

	if (!isRecord(x.slots)) return false;

	// coercion

	for (const [k, v] of Object.entries(x.slots)) {
		if (typeof v !== 'string') delete x.slots[k];
	}
...
function coerceOutfitDiff(x: unknown): x is OutfitCachedDiff {
	if (!isRecord(x)) return false;
