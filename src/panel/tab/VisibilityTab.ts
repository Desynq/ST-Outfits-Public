...
export class VisibilityTab extends PanelTab {
	...
	private renderTagInputs(contentArea: HTMLDivElement): void {
		// only character panels can have a custom tag and attributes
		if (!(this.panel instanceof CharOutfitPanel)) return;
		...
		const apply = (): void => {
			...
			const result = settings.setFullSummaryTag(
				tagInput.value,
				attrInput.value
			);
		...
		wrapper.append(
			el('label', { text: 'Panel Tag:' }),
			tagInput,
			el('label', { text: 'Panel Attributes:' }),
			attrInput,
			resetBtn,
			messageEl
		);
