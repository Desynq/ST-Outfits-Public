...
interface SlotValueTextDeps {
	showPromptModal(promptText: string): Promise<void>;
	expandValue(): void;
}
...
export class SlotValueText {
	...
	public constructor(
		private readonly deps: SlotValueTextDeps
	) { }
	...
	private parseInline(text: string): Node[] {
		...
		while ((match = regex.exec(text)) !== null) {
			...
			const [
				...
				strikeContent,
				starItalicContent,
				underscoreItalicContent
			] = match;

			let elNode: HTMLElement | null = null;
	...
	private createSpoilerSpan(macro: MacroMatch): HTMLSpanElement {
		const span = el('span', {
			className: 'slot-macro-span',
			text: macro.full
		});
